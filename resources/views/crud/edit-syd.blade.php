...
<main id="main" class="main">
    <section class="section">
        <div class="card">
            <div class="card-body">
                <h1>Edit Surat Yang Dipersamakan</h1>
                ...
                @endif
                <form action="{{ route('surat-dipersamakan.update', $temuan->id) }}" method="POST" enctype="multipart/form-data" id="editTemuanForm">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" id="no_lhp" name="no_lhp" value="{{ old('no_lhp', $temuan->no_lhp) }}">
                            </div>
                            <div class="form-group">
                                <label for="tgl_lhp">Tanggal LHP</label>
                                <input type="date" class="form-control" id="tgl_lhp" name="tgl_lhp" value="{{ old('tgl_lhp', $temuan->tgl_lhp) }}">
                            </div>
                            <div class="form-group">
                                <label for="informasis_id">Informasi</label>
                                <select class="form-control" id="informasis_id" name="informasis_id">
                                    @foreach ($informasis as $informasi)
                                        <option value="{{ $informasi->id }}" {{ old('informasis_id', $temuan->informasis_id) == $informasi->id ? 'selected' : '' }}>
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control" id="opd_id" name="opd_id">
                                    @foreach ($opds as $opd)
                                        <option value="{{ $opd->id }}" {{ old('opd_id', $temuan->opd_id) == $opd->id ? 'selected' : '' }}>
                                            {{ $opd->opd_name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control" id="pegawai_id" name="pegawai_id">
                                    @foreach ($pegawais as $pegawai)
                                        <option value="{{ $pegawai->id }}" {{ old('pegawai_id', $temuan->pegawai_id) == $pegawai->id ? 'selected' : '' }}>
                                            {{ $pegawai->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control" id="status_id" name="status_id">
                                    @foreach ($statuses as $status)
                                        <option value="{{ $status->id }}" {{ old('status_id', $temuan->status_id) == $status->id ? 'selected' : '' }}>
                                            {{ $status->status }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control" id="penyedia_id" name="penyedia_id">
                                    @foreach ($penyedias as $penyedia)
                                        <option value="{{ $penyedia->id }}" {{ old('penyedia_id', $temuan->penyedia_id) == $penyedia->id ? 'selected' : '' }}>
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" id="nilai_rekomendasi_display" oninput="formatRupiah(this)" value="{{ old('nilai_rekomendasi', number_format($temuan->nilai_rekomendasi, 0, ',', '.')) }}">
                                <input type="hidden" id="nilai_rekomendasi" name="nilai_rekomendasi" value="{{ old('nilai_rekomendasi', $temuan->nilai_rekomendasi) }}">
                            </div>
                            <div class="form-group">
                                <label for="jenistemuan_id">Jenis Temuan</label>
                                <select class="form-control" id="jenistemuan_id" name="jenistemuan_id">
                                    ...
                                    @foreach ($jenisTemuans as $jenisTemuan)
                                        <option value="{{ $jenisTemuan->id }}" {{ old('jenistemuan_id', $temuan->jenistemuan_id) == $jenisTemuan->id ? 'selected' : '' }}>
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" id="jumlah_jaminan" name="jumlah_jaminan" value="{{ old('jumlah_jaminan', $temuan->jumlah_jaminan) }}">
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="form-group">
                                <label for="no_sdp">No Surat Dipersamakan</label>
                                <input type="text" class="form-control" id="no_sdp" name="no_sdp" value="{{ old('no_sdp', $temuan->no_sdp) }}">
                            </div>
                            <div class="form-group">
                                ...
                                <select class="form-control" id="statustgr_id" name="statustgr_id">
                                    @foreach ($statustgrs as $statustgr)
                                        <option value="{{ $statustgr->id }}" {{ old('statustgr_id', $temuan->statustgr_id) == $statustgr->id ? 'selected' : '' }}>
                                            {{ $statustgr->tgr_name }}
                                        </option>
                                    @endforeach
                                </select>
