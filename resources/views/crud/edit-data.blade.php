    ...
    <main id="main" class="main">
                            ...
                            <div class="col-md-6">
                                    ...
                                    <select class="form-control" id="informasis_id" name="informasis_id">
                                        @foreach ($informasis as $informasi)
                                            <option value="{{ $informasi->id }}"
                                                {{ $data->informasis_id == $informasi->id ? 'selected' : '' }}>
                                    ...
                                    <select class="form-control" id="status_id" name="status_id">
                                        @foreach ($statuses as $status)
                                            <option value="{{ $status->id }}"
                                                {{ $data->status_id == $status->status ? 'selected' : '' }}>
                                                {{ $status->status }}
                                            </option>
                                        @endforeach
                                    </select>
                                    ...
                                    <select class="form-control" id="pegawai_id" name="pegawai_id">
                                        @foreach ($pegawais as $pegawai)
                                            <option value="{{ $pegawai->id }}"
                                                {{ $data->pegawai_id == $pegawai->id ? 'selected' : '' }}>
                                                {{ $pegawai->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                    ...
                                    <select class="form-control" id="penyedia_id" name="penyedia_id">
                                        @foreach ($penyedias as $penyedia)
                                            <option value="{{ $penyedia->id }}"
                                                {{ $data->penyedia_id == $penyedia->id ? 'selected' : '' }}>
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control" id="statustgr_id" name="statustgr_id">
                                        <option value="" {{ is_null($data->statustgr_id) ? 'selected' : '' }}>Pilih
                                            Status TGR</option>
                                        @foreach ($statustgrs as $statustgr)
                                            <option value="{{ $statustgr->id }}"
                                                {{ $data->statustgr_id == $statustgr->id ? 'selected' : '' }}>
                                                {{ $statustgr->tgr_name }}
                                            </option>
                                        @endforeach
                                    </select>
                                    ...
                                    <select class="form-control" id="jenistemuan_id" name="jenistemuan_id">
                                        @foreach ($jenisTemuans as $jenisTemuan)
                                            <option value="{{ $jenisTemuan->id }}"
                                                {{ $data->jenistemuan_id == $jenisTemuan->id ? 'selected' : '' }}>
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control" id="nilai_rekomendasi_display"
                                        ...
                                        value="Rp {{ number_format($data->nilai_rekomendasi, 2, ',', '.') }}">
                                    <input type="hidden" id="nilai_rekomendasi" name="nilai_rekomendasi"
                                        value="{{ $data->nilai_rekomendasi }}">
                                </div>
                            ...
                            <div class="col-md-6">
                                ...
                                <div class="form-group">
                                    ...
                                    @if ($data->bukti_pembayaran)
                                        <a href="{{ Storage::url($data->bukti_pembayaran) }}" target="_blank">Lihat Bukti
                                            Pembayaran</a>
                                    @endif
                                </div>
                            </div>
        ...
        <script>
            function formatRupiah(element) {
                ...
                // Update the hidden input with the numerical value, keeping two decimal places
                let numericValue = rupiah.replace(/\./g, '').replace(',', '.');
                document.getElementById('nilai_rekomendasi').value = numericValue;
            }


            $(document).ready(function() {
                $('#informasis_id, #opd_id, #status_id, #pegawai_id, #penyedia_id, #statustgr_id, #jenistemuan_id')
                    .select2();
            });
        </script>
        <script>
